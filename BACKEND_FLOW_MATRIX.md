...
## 9. Stripe-Spende (Fehlgeschlagen) ⚠️

| Schritt       | Komponente                                             | Detail                                                                                           |
| ------------- | ------------------------------------------------------ | ------------------------------------------------------------------------------------------------ |
| Trigger       | Stripe Webhook                                         | Event `payment_intent.payment_failed`                                                            |
| Validierung   | Stripe Signature Verification + Idempotenz-Vorpruefung | Ungueltige Signatur -> 400, Duplicate Event -> kein Reprocessing                                 |
| Persistenz    | `UPDATE payment_intents SET status = 'failed'`         | Status wird auf `failed` gesetzt, `updated_at` aktualisiert                                      |
| CRM-Sync      | —                                                      | —                                                                                                |
| Email         | `donation_failed` Template                             | Benachrichtigung mit Fehlergrund + Retry-Link an Spender                                         |
| Testabdeckung | `apps/api/tests/test_payment_flow.py`                  | Failed-, Canceled- und Duplicate-Verhalten sind abgedeckt                                        |
| ⚠️ Restlücke  | Kein CRM-Sync im Failure-Fall                          | Fehlgeschlagene Zahlungen erzeugen derzeit keine Contribution-/Status-Synchronisation in CiviCRM |
| ⚠️ Restlücke  | Kein Ops-/Admin-Alert                                  | Kein separater Slack-/n8n-/Admin-Alarm bei gehäuften Stripe-Fehlschlaegen                        |
...
| 9   | Stripe-Spende (Fehlgeschlagen) | ⚠️ Implementiert, aber ohne CRM-/Ops-Folgeorchestrierung |
...
**Gesamt:** 18 komplett, 5 mit Lücken, 0 fehlend, 2 mit Hinweisen
