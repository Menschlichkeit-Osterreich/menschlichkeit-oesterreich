...
Fuer dieses Repository gilt zusaetzlich der kanonische Analysevertrag ueber `.github/instructions/core/analysis-planning.instructions.md`.
Nutze `.github/ai-registry.json` als machine-readable Quelle fuer aktive, Adapter-, Vendor- und Legacy-Artefakte.
Bei groesseren Plaenen muss der offene Backlog gegen die kanonische GitHub-Abfrage `state:open repo:${owner}/${repository} sort:updated-desc` gegengeprueft werden.
Historische `.copilot-tracking/*`-Ablagen sind in diesem Repository kein verpflichtender Governance-Pfad, wenn der Repo-Vertrag etwas anderes vorgibt.
