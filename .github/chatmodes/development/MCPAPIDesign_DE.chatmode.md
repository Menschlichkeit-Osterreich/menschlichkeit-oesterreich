...
# apps/api/app/routers/donations.py
...
async def create_donation(donation: DonationCreate): # Sanitize PII before logging
...
def test*rate_limiting():
for * in range(11): # Exceed 10/minute limit
...
# apps/api/app/routers/admin_crm.py
...
async def sync_donation_to_civicrm(donation: Donation): # Find or create contact
...
@router.post("/donations")
async def create_donation(donation: DonationCreate): # Save to local DB
...
# apps/api/app/middleware/**init**.py
