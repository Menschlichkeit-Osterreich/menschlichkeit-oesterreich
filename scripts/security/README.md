...
### incident-network-timeout.ps1

Reproducible collector for source-/path-selective timeout investigations against a specific host/IP.

**Usage:**

```bash
pwsh -File scripts/security/incident-network-timeout.ps1 -CaseName selective-timeout-5.183.217.146 -EgressLabel kabelplus
pwsh -File scripts/security/incident-network-timeout.ps1 -CaseName selective-timeout-5.183.217.146 -EgressLabel lte
```

**Features:**

- Captures current public source IP with provider fallback
- Captures DNS A, PTR and AAAA evidence
- Runs reproducible `curl` and `ssh` connectivity checks
- Regenerates a case-level run matrix and provider ticket draft after every run

**Artifacts:**

- Writes artifacts into `quality-reports/incident-network-timeout/<case>/`
- Stores one subdirectory per run under `runs/<timestamp>-<egress>/`
- Mirrors the latest summary, ticket draft and metadata into `reports/incident-network-timeout/<case>/` for easy IDE access
