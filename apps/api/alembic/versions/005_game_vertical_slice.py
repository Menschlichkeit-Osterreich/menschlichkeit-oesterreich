...
def upgrade() -> None:
    op.create_table(
        "game_profiles",
        sa.Column("member_id", UUID(), sa.ForeignKey("members.id", ondelete="CASCADE"), primary_key=True),
        sa.Column("selected_role", sa.Text(), nullable=False, server_default="buerger"),
        sa.Column("current_world_id", sa.Text(), nullable=False, server_default="gemeinde"),
        ...
        sa.Column("world_state", JSONB(), nullable=False, server_default=sa.text("'{}'::jsonb")),
    ...
    op.create_table(
        ...
        sa.Column("used_signature_action", sa.Boolean(), nullable=False, server_default="false"),
        sa.Column("stats_delta", JSONB(), nullable=False, server_default=sa.text("'{}'::jsonb")),
        sa.Column("world_delta", JSONB(), nullable=False, server_default=sa.text("'{}'::jsonb")),
        sa.Column("world_state_after", JSONB(), nullable=False, server_default=sa.text("'{}'::jsonb")),
        sa.Column("completed_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.text("NOW()")),
        sa.UniqueConstraint("member_id", "scenario_id", name="uq_game_progress_member_scenario"),
    )
