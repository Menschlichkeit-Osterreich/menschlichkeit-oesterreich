...
def upgrade() -> None:
    ...
    op.create_table('events',
        ...
        sa.Column('start_datum',     sa.DateTime(timezone=True), nullable=False),
        sa.Column('end_datum',       sa.DateTime(timezone=True), nullable=True),
        sa.Column('max_teilnehmer',  sa.Integer(),    nullable=True),
        sa.Column('kategorie',       sa.Text(),       server_default='Allgemein'),
        sa.Column('ist_oeffentlich', sa.Boolean(),    server_default='true'),
        sa.Column('ersteller_id',    UUID(),          nullable=False),
        sa.Column('created_at',      sa.DateTime(timezone=True), server_default=sa.text('NOW()')),
        sa.Column('updated_at',      sa.DateTime(timezone=True), server_default=sa.text('NOW()')),
    )
    ...
    op.create_table('event_rsvps',
        ...
        sa.Column('status',     sa.Text(),  server_default='angemeldet'),
        sa.Column('created_at', sa.DateTime(timezone=True), server_default=sa.text('NOW()')),
    ...
    op.create_table('forum_threads',
        sa.Column('id',          UUID(),     primary_key=True, server_default=sa.text('gen_random_uuid()')),
        sa.Column('category_id', UUID(),     sa.ForeignKey('forum_categories.id'), nullable=True),
        sa.Column('titel',       sa.Text(),  nullable=False),
        sa.Column('inhalt',      sa.Text(),  nullable=False),
        sa.Column('autor_id',    UUID(),     nullable=False),
    ...
    op.create_table('forum_posts',
        sa.Column('id',         UUID(),     primary_key=True, server_default=sa.text('gen_random_uuid()')),
        sa.Column('thread_id',  UUID(),     sa.ForeignKey('forum_threads.id', ondelete='CASCADE'), nullable=False),
        sa.Column('inhalt',     sa.Text(),  nullable=False),
        sa.Column('autor_id',   UUID(),     nullable=False),
        sa.Column('created_at', sa.DateTime(timezone=True), server_default=sa.text('NOW()')),
        sa.Column('updated_at', sa.DateTime(timezone=True), server_default=sa.text('NOW()')),
    ...
    op.add_column('newsletter_subscriptions',
        sa.Column('token_created_at', sa.DateTime(timezone=True), server_default=sa.text('NOW()')),
    )
