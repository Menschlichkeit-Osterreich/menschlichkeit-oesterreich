...
INVOICE_CONFIG = {
    ...
    "issuer": {
        ...
        "iban": get_secret(
            "SEPA_CREDITOR_IBAN", "", bsm_key="api/SEPA_CREDITOR_IBAN"
        ).strip(),
        "bic": get_secret(
            "SEPA_CREDITOR_BIC", "", bsm_key="api/SEPA_CREDITOR_BIC"
        ).strip(),
        "bank": os.getenv("FINANCE_ISSUER_BANK", ""),
...
def calculate_due_date(issue_date: date, days: int | None = None) -> date:
...
async def trigger_n8n_webhook(webhook_path: str, payload: Dict[str, Any]) -> bool:
    ...
    try:
        async with httpx.AsyncClient(timeout=10.0) as client:
            ...
            logger.info(
                "n8n webhook triggered: %s → %s", webhook_path, resp.status_code
            )
            return True
...
class InvoiceService:
    ...
    async def create_and_send_invoice(
        self, contact: Dict[str, Any], items: List[Dict[str, Any]], **kwargs
    ) -> Dict[str, Any]:
        ...
        logger.info(
            "Invoice created and n8n workflow triggered for contact %s",
            contact.get("id"),
        )
        return invoice_data
...
class DunningService:
    ...
    def get_next_dunning_level(self, current_level: Optional[str]) -> Optional[str]:
        ...
        try:
            ...
            return (
                self.DUNNING_LEVELS[idx + 1]
                if idx + 1 < len(self.DUNNING_LEVELS)
                else None
            )
        except ValueError:
            return "reminder_1"
    ...
    async def run_dunning_check(
        self, overdue_invoices: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        ...
        for invoice in overdue_invoices:
            ...
            last_date = (
                date.fromisoformat(invoice["last_dunning_date"])
                if invoice.get("last_dunning_date")
                else None
            )
...
class DonationService:
    ...
    def create_receipt_data(
        ...
        return {
            ...
            "purpose": donation.get(
                "purpose", INVOICE_CONFIG["receipt"]["purpose_default"]
            ),
            "donor": {
